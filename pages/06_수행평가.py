...
def get_match_details(match_ids, target_puuid):
    ...
    while idx < len(match_ids):
        ...
        # 429 과부하 오류 발생 시 3초 동안 대기 후 해당 경기 재요청
        ...
        idx += 1
        progress_bar.progress(idx / len(match_ids), text=f"📊 총 {len(match_ids)}경기 중 {idx}번째 경기 정밀 분석 중...")
...
st.title("🏆 LoL AI 마스터 프로파일러 5.0")
st.caption("프로페셔널 이스포츠 분석 툴킷을 기반으로 유저의 강점과 플레이 스타일을 다각도로 정밀 리포팅합니다.")

# 사이드바 구성 및 가이드 배치
st.sidebar.header("🔍 대상 플레이어 입력")

# ✨ [업그레이드]: 주현빈, 최강우 유저 정보를 가이드 양식에 추가 포함
with st.sidebar.expander("🌟 추천! 검색 가이드", expanded=True):
    st.markdown("""
    정보를 복사해서 아래 입력창에 바로 확인해 보세요!
    
    * **주현빈**
      * 닉네임: `hbjoo` | 태그: `7063`
    * **최강우**
      * 닉네임: `bossbaby` | 태그: `5171`
    * **페이커 (Faker)**
            ...
            if puuid:
                ...
                if not match_ids:
                    ...
                    if df.empty:
                        ...
                        fig_champ.update_layout(yaxis=dict(autorange="reversed"))
