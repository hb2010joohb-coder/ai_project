...
# 리눅스 서버에는 나눔글꼴 등이 없을 수 있으므로 기본 폰트를 지정하되, 
# 그래프 구성 요소에 영문을 병행 표기하여 가독성을 높였습니다.
...
def load_data():
    ...
    # 시도할 인코딩 목록 (utf-8 실패 시 cp949 등으로 자동 전환)
...
if df is not None:
    ...
    selected_day = st.sidebar.selectbox("일(Day)을 선택하세요", list(range(1, 31)), index=14)   # 기본값 15일
...
ax.plot(filtered_df['연도'], filtered_df['최저기온(℃)'], color='lightblue', marker='o', markersize=3, label='Min Temp (최저기온)')
        ...
        with col1:
            ...
            st.metric(label="역대 가장 높았던 최고 기온", value=f"{max_row['최고기온(℃)']} ℃", delta=f"{int(max_row['연도'])}년")
            
        with col2:
            min_row = filtered_df.loc[filtered_df['최저기온(℃)'].idxmin()]
            st.metric(label="역대 가장 낮았던 최저 기온", value=f"{min_row['최저기온(℃)']} ℃", delta=f"{int(min_row['연도'])}년")
